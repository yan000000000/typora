...
### 大Θ Big Theta 渐进准确界符号

​    在五个记号中要求最严格，因为它**同时描述了上界和下界**。如果说一个算法$f(n)$的时间复杂度是Θ(h(n))，意味着既存在常数$C_1$和$C_2$，也存在一个$N_0$，使得当n ≥ $N_0$时，$C_1$\*h(n) ≤ $f(n)$ ≤ $C_2$​\*h(n)。这意味着算法的运时间随输入大小n的增加，其增长率被h(n)紧密包围。
